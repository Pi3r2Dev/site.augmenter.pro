...
- **Articles `/blog` — primitives de lecture** : en restructurant un article, en profiter pour poser/mettre à jour les repères du **fil de mémoire** (`<Memo>` 4-8, mix idea/num/link) et les `<Callout>`/`<PullQuote>`/`<KeyTakeaways>`. **Retirer tout encadré TL;DR inline** (le `tldr` du catalog s'affiche auto en tête). Réf. : [`.claude/templates/seo/article-primitives.md`](.claude/templates/seo/article-primitives.md)
