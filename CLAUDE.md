...
- **[src/data/capabilities.ts](src/data/capabilities.ts) — matrice de capacités = SOURCE DE VÉRITÉ** de l'outil de la section 02 de `/` (« ce que l'IA sait faire chez vous »). Deux axes (métier × outil en place) croisés avec relâchement progressif, trois verdicts (`production` / `cadrer` / `pas-encore`). ⚠️ Contenu **curé à la main**, jamais généré : chaque phrase doit être défendable en rendez-vous et chaque `proof` vérifiable. Le verdict `pas-encore` est le principal signal E-E-A-T de la page — ne pas le vider. Le CTA dépose le contexte via [`src/lib/quote-prefill.ts`](src/lib/quote-prefill.ts), seul endroit qui connaît la clé `localStorage` du wizard.
